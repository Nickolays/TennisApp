...
class HomographyMatrix:
    ...
    def __init__(self, calc_n_frame=None) -> None:
        """
            calc_n_frame: Every n frames we update our homorgraphy matrix. If None, we don't cahnge
        """
        self.matrix = None
        self.calc_n_frame = calc_n_frame

    def courtMap(self, frame, top_left, top_right, bottom_left, bottom_right):
        """
            TODO: Create new dataclass for court keypoints
        """
        pts1 = float32([[top_left, top_right, bottom_left, bottom_right]])
        pts2 = float32([self.courtTL, self.courtTR, self.courtBL, self.courtBR])
        M = getPerspectiveTransform(pts1,pts2)
        dst = warpPerspective(frame,M,(self.width, self.height))
        return dst, M

    def showLines(self, frame):
        rectangle(frame, (0,0),(self.width, self.height),(255,255,255),6)
    ...
    def __call__(self, frames: List, court_detections: List, n_skip_frames: int) -> List:
        ...
        # Here we save 
        outputs = [] 
        for indx in range(1, len(frames), self.calc_n_frame):
